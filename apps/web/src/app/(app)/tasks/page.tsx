...
export default function TasksPage() {
  ...
  async function load() {
    try {
      setTasks(
        (await apiFetch<{ data: Task[] }>(`/tasks${status === "all" ? "" : `?status=${status}`}`))
          .data,
      );
      setError(null);
    } catch (e) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    const dueAtEntry = f.get("dueAt");
    const dueAt =
      typeof dueAtEntry === "string" && dueAtEntry ? new Date(dueAtEntry).toISOString() : undefined;
    try {
  ...
  return (
    <div className="grid gap-6">
      <PageHeader
        ...
        description="Organize lembretes de compra, contas vencendo, estoque crítico e fechamento de caixa."
        actions={
      ...
      <section className="grid gap-4 xl:grid-cols-[340px_minmax(0,1fr)]">
        ...
        <div className="grid gap-3">
          ...
          {tasks.length ? (
            ...
            <Card>
              <CardContent>
                <p className="text-sm text-slate-500">Nenhuma tarefa neste filtro.</p>
              </CardContent>
            </Card>
          )}
        </div>
