...
type TicketDetail = {
  ...
  messages: Array<{ id: string; authorKind: string; authorName: string; body: string; createdAt: string }>;
};
  ...
  return (
    <div className="grid gap-6">
      <PageHeader
        ...
        actions={<Button variant="secondary" icon={<RefreshCw size={16} />} onClick={() => void load()}>Atualizar</Button>}
      />
          ...
          <Card>
            <CardContent>
              <form className="grid gap-3" onSubmit={(event) => void create(event)}>
                ...
                <Input name="subject" label="Assunto" required placeholder="Ex.: Dúvida no fechamento de caixa" />
                <label className="grid gap-1 text-sm font-medium text-[var(--brand-primary)]">
                  Descrição
                  <textarea name="description" required rows={5} className="rounded-md border border-[var(--brand-border)] px-3 py-2 text-sm outline-none focus:border-[var(--brand-secondary)]" placeholder="Conte o que aconteceu, em qual tela e qual resultado esperado." />
                </label>
          ...
          <Card>
            <CardContent className="grid gap-3">
              <div className="grid grid-cols-[1fr_150px] gap-3">
                <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar chamados" aria-label="Buscar chamados" />
                <Button variant="secondary" onClick={() => void load()}>Buscar</Button>
              </div>
              <Select value={status} onChange={(event) => setStatus(event.target.value)} options={statuses} aria-label="Status" />
            </CardContent>
          </Card>
        </div>
                ...
                <div className="grid gap-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button variant="secondary" onClick={() => void closeTicket("resolved")}>Resolver</Button>
                      <Button variant="secondary" onClick={() => void closeTicket("closed")}>Fechar</Button>
                    </div>
                  </div>
                  <div className="grid gap-3">
                    {detail.messages.map((message) => (
                      <article key={message.id} className={`rounded-md border p-3 ${message.authorKind === "platform_user" ? "border-blue-100 bg-blue-50" : "border-[var(--brand-border)] bg-white"}`}>
                        ...
                        <p className="mt-2 whitespace-pre-wrap text-sm text-[var(--brand-primary)]">{message.body}</p>
                      </article>
                    ))}
                  </div>
                  {detail.ticket.status !== "closed" ? (
                    <form className="grid gap-3 border-t border-[var(--brand-border)] pt-4" onSubmit={(event) => void reply(event)}>
                      <label className="grid gap-1 text-sm font-medium text-[var(--brand-primary)]">
                        Responder
                        <textarea name="body" required rows={4} className="rounded-md border border-[var(--brand-border)] px-3 py-2 text-sm outline-none focus:border-[var(--brand-secondary)]" />
                      </label>
                      <Button type="submit" icon={<Send size={16} />}>Enviar resposta</Button>
                    </form>
                  ) : null}
                </div>
              ) : (
                <EmptyState icon={<Headset size={18} />} title="Selecione um chamado" description="O histórico e as respostas aparecem aqui." />
              )}
            </CardContent>
          </Card>
        </div>
