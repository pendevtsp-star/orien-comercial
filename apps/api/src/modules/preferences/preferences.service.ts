...
type PreferenceRow = {
  theme: string; colorMode: string; sidebarMode: string; density: string; startPage: string; dateFormat: string;
  reduceMotion: boolean; notifyInApp: boolean; notifyEmail: boolean; quietHoursStart: string | null; quietHoursEnd: string | null;
  favoriteRoutes: string[]; dashboardWidgets: string[];
};
...
export class PreferencesService {
  ...
  async get(userId: string) {
    ...
    const result = await this.db.pool.query<PreferenceRow>(
  ...
  async update(userId: string, input: PreferenceInput) {
    const result = await this.db.pool.query<{ user_id: string }>(
